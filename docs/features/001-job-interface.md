...
The goal is not just to rename `process_ingest_folder` into a trait method. The worker needs a real job execution contract that gives every job the same lifecycle: claim, decode payload, run with context, refresh heartbeat, report failure, and complete or retry according to policy.

## Problems To Avoid

1. A trait like `fn process(&self, pool: &DbPool, model_id: i64)` is too weak. It hides the job row, payload, job id, shutdown behavior, and error semantics from the handler.
2. Handlers must remain fallible. Logging and returning `()` makes data loss indistinguishable from success.
3. Heartbeat failures are not harmless. They must be logged with job id and enough context, and future retry/staleness behavior must be able to reason about them.
...
`ShutdownToken` can start as a small wrapper around an atomic flag or channel receiver. It does not need to support full process shutdown immediately, but the trait should not block adding it later.
...
This means every job implementation receives the claimed queue entry and shared runtime context. Future handlers can inspect `ctx.entry.id`, deserialize their payload, access the pool, and check shutdown state without expanding the trait signature again.
...
Move heartbeat logic into a worker-owned guard:
...
Heartbeat update failures should be returned from the heartbeat thread and logged by the dispatcher with `job_id`. Do not silently ignore them.
...
3. Start a heartbeat guard for `entry.id`.
4. Call `handler.process(JobContext { ... })`.
5. Stop heartbeat guard and surface heartbeat stop errors.
...
7. Replace direct `println!` / `eprintln!` in new code with structured logging when a logging crate is available.

## Required Tests

1. A custom blocking test handler proves heartbeat updates while the handler is still running.
...
5. A future `EmbedDocument` skeleton can implement the trait without changing heartbeat code.

## Acceptance Criteria

1. No job handler calls `repository::update_heartbeat` directly.
2. Every handler implements the same `process(ctx) -> Result<()>` contract.
3. Dispatcher-owned heartbeat is directly tested with a controllable handler.
