...
#[tokio::test]
async fn ingest_folder_git_skips_tracked_files_matching_ignore_rules() {
    let db = TempDb::new();
    let now = current_time();
    let model_id = register_model(&db.pool(), now, "git_ignored_tracked");

    let ingest_dir = db.create_dir("git_ignored_tracked");
    init_git_repo(&ingest_dir);

    let tracked_file = ingest_dir.join("docs").join("keep.md");
    let ignored_tracked_file = ingest_dir.join("docs").join("temp").join("skip.md");
    std::fs::create_dir_all(ignored_tracked_file.parent().unwrap()).unwrap();
    std::fs::write(ingest_dir.join(".gitignore"), "docs/temp/\n").unwrap();
    std::fs::write(&tracked_file, "keep\n").unwrap();
    std::fs::write(&ignored_tracked_file, "skip\n").unwrap();
    stage_git_paths(&ingest_dir, &[tracked_file.as_path()]);
    run_git(
        &ingest_dir,
        &["add", "-f", "--", &display_path(&ignored_tracked_file)],
    );
...
#[tokio::test]
async fn ingest_folder_git_deletes_existing_documents_matching_ignore_rules() {
    let db = TempDb::new();
    let now = current_time();
    let model_id = register_model(&db.pool(), now, "git_ignored_cleanup");

    let ingest_dir = db.create_dir("git_ignored_cleanup");
    init_git_repo(&ingest_dir);
    ...
    let ignored_doc_id = insert_document(&db.pool(), now, &ignored_tracked_file);
    let payload_json = ingest_payload(&ingest_dir);
    let conn = db.pool().get().unwrap();
    repository::enqueue_job(&conn, &JobType::IngestFolderGit, &payload_json, now).unwrap();
    let mut conn = db.pool().get().unwrap();
    ...
    let ignored_count: i64 = conn
        ...
        .unwrap();
    assert_eq!(ignored_count, 0, "ignored document row should be deleted");

    let kept_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM documents WHERE filepath = ?1",
            [tracked_file.canonicalize().unwrap().display().to_string()],
            |row| row.get(0),
        )
        .unwrap();
    assert_eq!(
        kept_count, 1,
        "non-ignored tracked file should remain indexed"
    );
}
